...
func solve_d12_2(filename string) int64 {
	...
	visitCave2(nil, start, &game)
	return game.result
}

func visitCave2(prev *Cave, current *Cave, game *CavesGame) {
	if current.end {
		game.result++
		return
	}
	...
	if current.visited {
		if !game.smallVisitedTwice {
			game.smallVisitedTwice = true
			defer func() { game.smallVisitedTwice = false }()
		} else {
			return
		}
	} else if !current.big {
